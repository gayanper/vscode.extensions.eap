...
for root, subdirs, files in os.walk(os.path.join(P2_REPOSITORY_ECLIPSE, 'plugins')):
    for file in files:
        if file.endswith('.jar'):
            name = file.split('_')[0]
            repo_jar_map.setdefault(name, []).append(os.path.abspath(os.path.join(root, file)))
...
for name, source_jar_path in source_jar_map.items():
    ...
    repo_jar_paths = repo_jar_map.get(name)
    if repo_jar_paths is not None:
        for repo_jar_path in repo_jar_paths:
